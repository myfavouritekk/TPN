...
def test_vid(session, m, vid_file, verbose=True):
  ...
  for ind, track in enumerate(tracks, start=1):
    ...
    # extract features
    f_state = session.run([m.initial_state])
    b_state = session.run([m.initial_backward_state])
    ...
    [m.cls_scores, m.bbox_pred, m.end_probs, m.final_state],
     {m.input_data: expend_feat[np.newaxis,:,:],
      m.initial_state: f_state[0],
      m.initial_backward_state: b_state[0]})
    ...
    track_res['scores_bilstm'] = cls_scores[:gt_len,:]
    track_res['end_bilstm'] = end_probs
    track_res['bbox_bilstm'] = bbox_pred.reshape((gt_len, -1, 4))
...
def main(args):
  ...
  with tf.Graph().as_default(), tf.Session() as session:
    ...
    with tf.variable_scope("model", reuse=None, initializer=None):
      m = BiTPNModel(is_training=False, config=config)
...
if __name__ == '__main__':
  parser = argparse.ArgumentParser('Extracting bidirectional recurrent features.')
  ...
  parser.add_argument('--combine', type=str, choices=['max', 'ave', 'concat'], default='concat',
      help='Bidirectional feature combine scheme. [concat]')
