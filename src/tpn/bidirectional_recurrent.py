...
flags.DEFINE_string("combine", "max,ave,concat", "Bidirectional feature combination scheme.")
...
def run_epoch(session, m, data, eval_op, init_state, epoch_idx, verbose=False):
  ...
  for step in xrange(m.iter_epoch):
    ...
    data_time = time.time()
    cost, cls_cost, bbox_cost, end_cost, state, b_state, _, global_norm = session.run(
        [m.cost, m.cls_cost, m.bbox_cost, m.end_cost,
        m.final_state, m.final_backward_state,
        eval_op,  m.global_norm],
         {m.input_data: x,
          m.cls_targets: cls_t,
          ...
          m.initial_backward_state: b_state})
    costs += cost
    tot_costs += cost
    cls_costs += cls_cost
  ...
  return tot_costs / m.iter_epoch, state, b_state
...
def main(_):
  ...
  with tf.Graph().as_default(), tf.Session() as session:
    ...
    with tf.variable_scope("model", reuse=None, initializer=initializer):
      m = BiTPNModel(is_training=True, config=config)
    with tf.variable_scope("model", reuse=True, initializer=initializer):
      mvalid = BiTPNModel(is_training=False, config=config)
    ...
    for i in range(config.max_epoch):
      ...
      if i == 0:
        ...
        b_state = m.initial_backward_state.eval()
      log.info("Epoch: {} Learning rate {:.03e}.".format(i+1,session.run(m.lr)))
      train_cost, state, b_state = run_epoch(session, m, train_data, m.train_op,
                             [state, b_state], i, verbose=True)
