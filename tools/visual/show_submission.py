...
from vdetlib.utils.visual import unique_colors
DATA_DIR = osp.join(this_dir, '../../dataset/ILSVRC2015/Data/VID')
IMAGESETS_DIR = osp.join(this_dir, '../../dataset/ILSVRC2015/ImageSets/VID')
SUBMISSION_DIR = osp.join(this_dir, '../../result_vid')
SCREENSHOT_DIR = osp.join(this_dir, '../../screenshot')
VIDEOSHOT_DIR = osp.join(this_dir, '../../videoshot')
...
PRESET_COLORS = map(lambda x: QtGui.QColor(*map(lambda y: 255 * y, x)), unique_colors(len(CLASS_NAMES)))
    ...
    with open(osp.join(IMAGESETS_DIR, subset + '.txt')) as f:
        ...
        for line in lines:
            ...
            fid_to_path[int(line[1])] = osp.join(
                DATA_DIR, subset.split('_')[0], line[0] + '.JPEG')
...
class MainWindow(QtGui.QMainWindow):
    ...
    def build_ui(self):
        ...
        file_menu.addAction('Open val_20', partial(self.open, subset='val_20'))
